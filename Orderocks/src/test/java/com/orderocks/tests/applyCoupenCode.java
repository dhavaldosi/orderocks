...
public class applyCoupenCode extends BaseClass {
	...
	public void CouponCode() {
		...
		home.clickZipCodeButton().selectGrocery().getVendorList().selectVendorFromList().clickSearchBar()
		 ...
		 home.clickSearchIcon().clickAddToCart().clickOnCart().selectDiscountCodesAndVouchers().enterGiftCardCode()
		 .ClickAdd().clickCheckOut().continueBillingAddress().selectSlotOfDelivery().continueShippingAddress();
